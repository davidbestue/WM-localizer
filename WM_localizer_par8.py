...
# all_df['index_c'] = all_df['index_c'].replace(['baseline', 'baselinech', 'checkerboard', 'target', 'delay', 'probe'], [1, 2, 3, 4, 5, 6])

# ### make the checkerboard last 10 seconds (design matrix with no consec durations)
# all_df.loc[all_df['condition']=='checkerboard', 'duration']=10
# all_df = all_df.reset_index()




all_df=pd.DataFrame(df['checkboard_time_ref'].values[0::2])
all_df.columns=['time']
all_df['index_c']=1
all_df['duration'] = df['probe_pres_ref'].values[1::2] - df['checkboard_time_ref'].values[0::2]
all_df['intercept']=1
all_df['condition']='checkerboard'



with open('wmmap8.par', 'w') as out:
